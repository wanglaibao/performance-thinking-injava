...
public class SynchronizedTest9 {
    ...
    private synchronized void testMethod() {
        System.out.println(Thread.currentThread().getName() + "testMethod start");
        while (true) {
            count++;
            ...
            if (count == 50) {  // 当count == 500 时，synchronized代码块会抛出异常
                int i = 1 / 0;
            }
        }
    }
    ...
    public static void main(String[] args) {
        SynchronizedTest9 test9 = new SynchronizedTest9();

        new Thread(() -> {
            test9.testMethod();
        }, "thread_1").start();

        // 执行到第5秒时，抛出 ArithmeticException
        // 如果抛出异常后，thread_2 会继续执行，就代表thread_2拿到了锁，即thread_1在抛出异常后释放了锁
